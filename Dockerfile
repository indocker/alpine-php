...
RUN apk add php7@php php7-fpm@php php7-json@php php7-zlib@php php7-xml@php php7-pdo@php php7-openssl@php \
    ...
    php7-curl@php php7-opcache@php php7-phar@php php7-ctype@php \ 
...
ADD config/etc-nginx/nginx.conf /etc/nginx/
ADD config/etc-nginx/server.conf /etc/nginx/
ADD config/etc-php7/php-fpm.conf /etc/php7/
ADD run.sh /
RUN chmod +x /run.sh
